...
pub fn get_connection(path: String) -> Result<Connection, ReleasrError> {
    ...
    let _res = conn.execute("CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY, version TEXT, version_int INTEGER, note TEXT, environment_name TEXT, completed_at TEXT NULL, created_at TEXT, modified_at TEXT)",[])?;
